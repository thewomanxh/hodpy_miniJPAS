...
class LuminosityFunction(object):
    ...
    def Phi(self, magnitude, redshift):
        ...
        """
        magnitude04 = magnitude + self.Q * (redshift - 0.4)

        # find interpolated number density at z=0.4
        log_lf04 = self.__Phi_z04(magnitude04)

        # shift back to redshift
        log_lf = log_lf04 + 0.4 * self.P * (redshift - 0.4)
        
        return 10**log_lf

    def __Phi_z04(self, magnitude):
        # returns a spline fit to the LF at z=0.4 (using the cumulative LF)
        mags = np.arange(0, -25, -0.001)
        phi_cums = self.Phi_cumulative(mags, 0.4)
...
class LuminosityFunctionSchechter(LuminosityFunction):
    ...
    def Phi(self, magnitude, redshift):
        ...
        M_star = self.M_star - self.Q * (redshift - 0.4)
...
class LuminosityFunctionTabulated(LuminosityFunction):
    ...
    def Phi_cumulative(self, magnitude, redshift):
        ...
        # shift magnitudes to redshift z=0.4
        magnitude04 = magnitude + self.Q * (redshift - 0.4)

        # find interpolated number density at z=0.4
        log_lf04 = self.__lf_interpolator(magnitude04)
...
class LuminosityFunctionTargetBGS(LuminosityFunctionTarget):
    ...
    def __init__(self, target_lf_file, miniJPAS_lf_file, lf_param_file, hod_bgs_simple):
    ...
    def __initialize_target_lf(self, target_lf_file, miniJPAS_lf_file, hod_bgs_simple):
        ...
        # save to file
        np.savetxt(target_lf_file, data)

        return LuminosityFunctionTabulated(target_lf_file, self.P,
                                           self.Q)
